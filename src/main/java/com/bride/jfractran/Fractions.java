...
public class Fractions extends ArrayList<Fraction> {
	 ...
	 * <code>d</code> en entrée.
	 * @param d La décomposition à utiliser pour le calcul.
	 * @param filter Le filtre pour filtrer les décompositions.
	 * @param limit Le nombre maximal de décompositions à calculer. Si
	 * <code>limit</code> est négatif, il n'y a pas de limite.
	 * @return Les décompositions calculées.
	 * @link http://scienceblogs.com/goodmath/2006/10/27/prime-number-pathology-fractra/
	 */
	public ArrayList<PrimeDecomposition> compute(PrimeDecomposition d,
			Filter filter, int limit) {
		ArrayList<PrimeDecomposition> res = new ArrayList<PrimeDecomposition>();
		int index = 0;
		int i = 0;
		int s = size();
	 ...
	 * @param d La décomposition à utiliser pour le calcul.
	 * @return Les décompositions calculées.
	 * @see Fractions#compute(PrimeDecomposition, Filter, int)
	 */
	public ArrayList<PrimeDecomposition> compute(PrimeDecomposition d) {
		return compute(d, Filter.DEFAULT, -1);
	}
}
